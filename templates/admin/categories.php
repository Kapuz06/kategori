...
jQuery(document).ready(function($) {
    ...
    $('#trendyol-category-search-input').on('keyup', function() {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('#trendyol-search-results').html('<p class="searching"><?php echo esc_js(__('Aranıyor...', 'trendyol-woocommerce')); ?></p>').show();
            },
    ...
    $('#wc-category-search-input').on('keyup', function() {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('#wc-search-results').html('<p class="searching"><?php echo esc_js(__('Aranıyor...', 'trendyol-woocommerce')); ?></p>').show();
            },
            success: function(response) {
                if (response.success && response.data.length > 0) {
                    let html = '<ul>';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        const attributeMappings = response.data.attribute_mappings || {};
                        ...
                        sortedAttributes.forEach(function(attr) {
                            ...
                            const currentMappingValue = attributeMappings[attrId] || '';
                            ...
                            wcAttributes.forEach(function(wcAttr) {
                                const selected = currentMappingValue === wcAttr.slug ? 'selected' : '';
                                html += `<option value="${wcAttr.slug}" ${selected}>${wcAttr.name}</option>`;
                            });
    ...
    $(document).on('click', '.save-attribute-mapping', function() {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                button.prop('disabled', true).text('<?php echo esc_js(__('Kaydediliyor...', 'trendyol-woocommerce')); ?>');
            },
            success: function(response) {
                if (response.success) {
                    alert(response.data.message);
                    // Kaydet butonunu güncelle
                    button.prop('disabled', false).text('<?php echo esc_js(__('Kaydet', 'trendyol-woocommerce')); ?>');
                } else {
                    alert(response.data.message);
                    button.prop('disabled', false).text('<?php echo esc_js(__('Kaydet', 'trendyol-woocommerce')); ?>');
                }
